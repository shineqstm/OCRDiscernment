package com.qstm.ocrdiscernment;
...
import com.qstm.ocrdiscernment.utils.FileUtils;
import com.qstm.ocrdiscernment.utils.ToastUtils;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private static final String API_KEY = "gPBBkqUS0OMiblGM3RQuDS3R";//替换成自己的api_key
    private static final String SECRET_KEY = "3O1H4hRcGEt8ft499P8URk0RYg3ACxTH ";//请替换成你的Secret key
    ...
    private  static final int REQUEST_CODE_VEHICLE_LICENSE=105; //行驶证的请求码
...
//        initAccessTokenWithAkSk();
    ...
    private void initView() {
        ...
        findViewById(R.id.vehicle_license_button).setOnClickListener(this);
    ...
    private void scanBank() {
        ...
        intent.putExtra(CameraActivity.KEY_OUTPUT_FILE_PATH,
                FileUtils.getSaveFile(getApplication(),CameraActivity.CONTENT_TYPE_BANK_CARD).getAbsolutePath());
    ...
    public void recVehicleLicense(String filePath) {
        ...
        OCR.getInstance().recognizeVehicleLicense(param, new OnResultListener<OcrResponseResult>() {
            ...
            @Override
            public void onError(OCRError error) {
                Log.i(TAG, "onError: 扫描行驶证错误  " + error.getMessage());
            }
        });
    }
    @Override
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_BANKCARD && resultCode == Activity.RESULT_OK) {
            recBankCard(FileUtils.getSaveFile(getApplicationContext(),CameraActivity.CONTENT_TYPE_BANK_CARD).getAbsolutePath());
        }
        //拍摄类型:驾驶证
        if (requestCode == REQUEST_CODE_DRIVING_LICENSE && resultCode == Activity.RESULT_OK) {
            recDrivingLicense(FileUtils.getSaveFile(getApplicationContext(),CameraActivity.CONTENT_TYPE_GENERAL).getAbsolutePath());
        }

        // 识别成功回调，行驶证识别
        if (requestCode == REQUEST_CODE_VEHICLE_LICENSE && resultCode == Activity.RESULT_OK) {
            recVehicleLicense(FileUtils.getSaveFile(getApplicationContext(),CameraActivity.CONTENT_TYPE_GENERAL).getAbsolutePath());
        }
    }
}
